...
class TicTacToe {
  ...
  checkColumns() {
    ...
    for (let column in this.tablerow) {
      if (this.arrayColumn(this.tablerow, column).toString() === this.CrossesWinningCombo.toString()) {
        this.Player1Output();
      } else if (this.arrayColumn(this.tablerow, column).toString() === this.NoughtsWinningCombo.toString()) {
        this.Player2Output();
      } else {
        this.checkRows();
      }
